...
let AnalyticsVideo = function (browser) {

  this.browser = browser;

  this.get = async function (videoId, period = null) {
    let that = this;
    let parameters = {};
    if (period) {
      parameters.period = period;
    }
    let response = await this.browser.get( '/analytics/videos/' + videoId + '?' + http_build_query(parameters));

    return new Promise(function (resolve, reject) {
      if (!that.browser.isSuccessfull(response)) {
        reject(response);
      } else {
        let analyticVideo = that.cast(response.body);
  ...
  this.search = async function (parameters = {}) {
    ...
    do {
      let response = await this.browser.get(
        '/analytics/videos?' + http_build_query(params));
...
AnalyticsVideo.prototype.castAll = function (collection) {
  let that = this;
  return array_map(function (data) {
    let analyticVideo = new AnalyticVideo();
...
AnalyticsVideo.prototype.cast = function (data) {
  if (!data) {
    return null;
  }
  let analyticVideo = new AnalyticVideo();
...
AnalyticsVideo.prototype.buildAnalyticEventsData = function (events) {
  let eventsBuilded = [];
