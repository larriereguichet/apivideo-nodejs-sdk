...
let Browser = function(username, apiKey, baseUri) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      request.post({
        ...
        if (error) {
          ...
          token = that.setAccessToken(body.token_type, body.access_token,
              body.refresh_token);
          resolve(token);
        }
      });
  ...
  this.isStillAuthenticated = async function(response) {
    ...
    if (response.statusCode === 401 && 'application/problem+json' ===
      ...
      this.headers['Authorization'] = token.token_type + ' ' +
          token.access_token;
