...
const Videos = function Videos(browser) {
  ...
  this.upload = async function upload(source, properties = {}, videoId = null) {
    ...
    return new Promise((async (resolve, reject) => {
      ...
      var uploadChunkSize = that.chunkSize;
      let lastResponse;
      for (let offset = 0; offset < length; offset += that.chunkSize) {
        //default the upload size to be as large as possible.
        uploadChunkSize = that.chunkSize;
        //BUT,if we are on the last chunk to be uploaded, the uploaded chunk must be reduced to match the reamining bytes in the file
        ...
        lastResponse = await that.browser.submit(
          ...
          buf,
          {
            'Content-Range': `bytes ${offset}-${offset + uploadChunkSize}/${length}`,
          },
        ).catch((error) => {
          fs.closeSync(file);
